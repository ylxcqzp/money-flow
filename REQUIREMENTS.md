...
- **交易列表**:
  - **条件筛选**: 支持按日期范围、账户、标签、分类进行复合过滤。
  - **分页查询**: 支持分页获取交易列表，便于大数据量场景展示。
...
### 3.5 统计与分析 (Statistics & Analysis)（规划）
当前后端未提供统计分析与报表导出接口，前端可视化与导出能力需依赖后续接口补充。
...
### 3.9 设置与数据 (Settings)
- **汇率管理**: 当前后端未提供汇率接口。
- **数据同步**: 所有业务数据通过 RESTful API 与后端同步。JWT Token、用户偏好等缓存于 LocalStorage。
