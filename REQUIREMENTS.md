...
### 3.5 周期性账单 (Recurring Transactions)
自动化处理固定频率的收支。
...
- **数据持久化**: 所有数据实时保存至 LocalStorage，刷新页面不丢失。
